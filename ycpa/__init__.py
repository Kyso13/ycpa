...
def fit(data, target, features=None, *, use_mcmc=True, **kwargs) -> FitResult:
    """Discover a power law Y = C * x1^a * x2^b * ... from a data frame.
...
class DiscoverResult:
    ...
    Low confidence means the formula may be wrong despite a high R^2.
    """
    ...
    def __repr__(self) -> str:
        ...
        return f"DiscoverResult({self.formula!r}, confidence={c}, R^2={r})"
...
def discover(data, target, features=None, *, atoms=None, max_terms=2,
    ...
    may be wrong despite a high R^2. Always take the confidence score into account.
    """
    if features is None:
        features = [c for c in data.columns if c != target]
    if atoms is None:
